...
func (rm *queryResmon) StreamInvoke(
	...
	ret, failures, err := prov.StreamInvoke(tok, args, stream)
	if err != nil {
		return errors.Wrapf(err, "invocation of %v returned an error", tok)
	}

	_, err = plugin.MarshalProperties(ret, plugin.MarshalOptions{Label: label, KeepUnknowns: true})
	if err != nil {
